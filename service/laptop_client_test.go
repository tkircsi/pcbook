...
func TestClientCreateLaptop(t *testing.T) {
	...
	// check if laptop is saved on the server
	other, err := laptopStore.Find(res.Id)
	require.NoError(t, err)
	require.NotNil(t, other)

	// check the saved laptop is the same we sent
...
func TestClientSearchLaptop(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		case 4:
			...
			laptop.Ram = &pb.Memory{Value: 16, Unit: pb.Memory_GIGYBYTE}
		...
		case 5:
			...
			laptop.Ram = &pb.Memory{Value: 64, Unit: pb.Memory_GIGYBYTE}
			expectedIDs[laptop.GetId()] = true
		}
